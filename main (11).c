...
int main() {
    ...
    // Define a 3D array where each row contains the unit limit and charge per unit
    float rateTable[1][4][2] = {
        {
    ...
    float chargePerUnit;
    if (unitsConsumed <= rateTable[0][0][0]) {
        chargePerUnit = rateTable[0][0][1];
    } else if (unitsConsumed <= rateTable[0][1][0]) {
        chargePerUnit = rateTable[0][1][1];
    } else if (unitsConsumed <= rateTable[0][2][0]) {
        chargePerUnit = rateTable[0][2][1];
    } else {
        chargePerUnit = rateTable[0][3][1];
    }
